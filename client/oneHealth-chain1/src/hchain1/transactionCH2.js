...
/*
  ...
  a library to create a new signed transaction for the HCHAIN2 family
...
const Address = require('../shared/hchain2address')
...
function createTransaction(opts) {
  ...
  const {
    ...
    // the name of the person we are targeting with this transaction
    personName,
  ...
  // construct the address for the person based on it's name
  const address = Address.personAddress(personName)

  // create the raw bytes for the transaction header
  const transactionHeaderBytes = protobuf.TransactionHeader.encode({

    // include the transactin family name and version 
    familyName: Address.HCHAIN2_FAMILY,
    familyVersion: Address.HCHAIN2_VERSION,
    ...
    // there are no dependencies for an HCHAIN2 transaction
    dependencies: [],
  ...
  return protobuf.Transaction.create({
    ...
    // the binary payload - HCHAIN2 uses utf8 CSV strings so we are
