...
/*
  ...
  a library to create a new signed transaction for the HCHAIN3 family
...
function createTransaction(opts) {
  ...
  const {
    ...
    // the name of the hchain3log we are targeting with this transaction
    hchain3logName,
  ...
  // construct the address for the hchain3log based on it's name
  const address = Address.hchain3logAddress(hchain3logName)

  // create the raw bytes for the transaction header
  const transactionHeaderBytes = protobuf.TransactionHeader.encode({

    // include the transactin family name and version 
    familyName: Address.HCHAIN3_FAMILY,
    familyVersion: Address.HCHAIN3_VERSION,
    ...
    // there are no dependencies for an HCHAIN3 transaction
    dependencies: [],
  ...
  return protobuf.Transaction.create({
    ...
    // the binary payload - HCHAIN3 uses utf8 CSV strings so we are
